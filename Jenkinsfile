pipeline {
    ...
    stages {
        stage('Build and test SDS Common') {
            steps {
                dir('sds/common') {
                    buildModules('Installing sds common dependencies')
        ...
        stage('Build SDS') {
            stages {
                stage('Build spine route lookup') {
                    steps {
                        dir('sds/spine-route-lookup') {
                ...
                stage('Build and Push image') {
                    ...
                    steps {
                        buildAndPushImage('${LOCAL_ROUTE_IMAGE_NAME}', '${ROUTE_IMAGE_NAME}', 'sds/spine-route-lookup/Dockerfile')
                    }
                }
            }
        }
